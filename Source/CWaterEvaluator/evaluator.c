...
int evaluateWaterC(int argc,char *argv[],float *nr_cost,int *env_cost, float *feas_violation)
{
   ...
   /* printf("A \n"); */ 
   for(crop=start_crop;crop<=sol_length;crop++) solution_x[crop]=atoi(argv
   [sol_length_index+crop]);
   for(month=start_month;month<=m;month++) envf[month]=atoi(argv[
   sol_length_index+sol_length+month]); 
   /* printf("B \n"); */ 
   ...
   /* printf("F \n"); */
   evaluate_all_objectives(water.c,water.cp,water.cw,solution_x,water.cgm,
   water.vcost,water.wreq,water.tenvf,envf,p,nr_cost,env_cost);
   /* printf("H \n"); */
   *feas_violation=evaluate_total_constraint_violation(solution_x,p,water.wreq,
   allocation,water.c,water.gw_proportion,water.gw_limit,water.tarea,water.
   area_limits,water.min_tarea);
