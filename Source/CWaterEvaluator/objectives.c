...
void evaluate_all_objectives(int c,int cp,int cw,int *solution_x,int *cgm,int *vsosts,float **wreq,int *tenvf,int *envf,float *p,float *nr_cost,int *env_cost)
{
   *nr_cost=evaluate_net_revenue(c,cp,cw,solution_x,cgm,wreq,vsosts,p);
   *env_cost=evaluate_environment_costs(tenvf,envf);
}


float evaluate_net_revenue(int c,int cp,int cw,int *solution_x,int *cgm,float **wreq,int *vcosts,float *p)
{
   int c_index,m_index,total1=0,total4=0;
   float total2=0,total3=0,net_revenue;
   for(c_index=start_crop;c_index<=c;c_index++) total1+=(cgm[c_index]*
   solution_x[c_index]); 
   ...
   for(m_index=start_month;m_index<=m;m_index++) if (p[m_index]<0) p[m_index]
   =0.0;
   ...
   solution_x[c_index]);
   printf("\n"); */
   for(c_index=start_crop;c_index<=c;c_index++) total4+=(vcosts[c_index]*
   solution_x[c_index]);
...
int evaluate_environment_costs(int *tenvf,int *envf)
{
  int m_index;
  float environment_cost=0;
  for(m_index=start_month;m_index<=m;m_index++) {
    int v = tenvf[m_index] - envf[m_index];
    if (v >= 0) {
      
      environment_cost+=(tenvf[m_index]-envf[m_index]);
    }
  }
